...
**`multiline: PropTypes.bool`:** when `true`, rows wrap (`white-space: normal; word-break: break-word`) instead of being clipped to a single line, restoring the wrapping behaviour of the original `react-inspector`. Row heights are then measured dynamically from the DOM (variable size) rather than using the fixed `rowHeight`, so virtualization is preserved. Recycled DOM slots are remeasured against their current node paths on live updates, including when a value switches between short, long, and explicit multiline strings. Defaults to `false` (the cheaper fixed-height path). Enable it when long values need to be fully visible without horizontal scrolling.
